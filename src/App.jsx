...
const PHOTO_STATUS_CLS = {
  "依頼中": "bg-orange-900/60 text-orange-300 border-orange-800/50",
  "撮影済": "bg-blue-900/60 text-blue-300 border-blue-800/50",
  "納品済": "bg-purple-900/60 text-purple-300 border-purple-800/50",
  "完了":   "bg-gray-800/80 text-gray-500 border-gray-700/50",
};
const EC_STATUS_CLS = {
  "受付":   "bg-slate-800 text-slate-400 border-slate-700/50",
  "対応中": "bg-blue-900/60 text-blue-300 border-blue-800/50",
  "完了":   "bg-gray-800/80 text-gray-500 border-gray-700/50",
};
                ...
                <section>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-semibold text-white">撮影依頼</h2>
                      {active.length > 0 && (
                    ...
                    <button onClick={() => { setShowPhotoForm(v => !v); setPhotoForm(EMPTY_PHOTO()); }}
                      className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg bg-orange-700/70 hover:bg-orange-600/70 text-orange-100 transition-colors cursor-pointer">
                      ＋ 追加
                    </button>
                  </div>
                  ...
                  {/* 追加フォーム */}
                  {showPhotoForm && (
                    <div className="bg-gray-900 border border-orange-900/50 rounded-xl p-5 space-y-4 mb-4">
                      <h3 className="text-xs font-semibold text-orange-300 uppercase tracking-wide">新規撮影依頼</h3>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        ...
                        <div className="col-span-2 md:col-span-3"><label className={labelCls}>撮影メモ</label>
                          <textarea className={`${inputCls} resize-none`} rows={2} value={photoForm.撮影メモ}
                      ...
                      <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowPhotoForm(false)}
                          className="bg-gray-700 hover:bg-gray-600 text-gray-300 px-4 py-2 rounded-lg text-sm cursor-pointer">キャンセル</button>
                        <button onClick={addPhotoRequest}
                          className="bg-orange-700 hover:bg-orange-600 text-white px-5 py-2 rounded-lg text-sm font-medium cursor-pointer">追加</button>
                      </div>
                    </div>
                  )}
                  ...
                  {active.length > 0 ? (
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b border-gray-800 text-left text-xs font-medium text-gray-500">
                              <th className="px-4 py-2.5 w-24">ステータス</th>
                              <th className="px-4 py-2.5">タスク名</th>
                              <th className="px-4 py-2.5 hidden md:table-cell w-28">案件</th>
                              <th className="px-4 py-2.5 hidden lg:table-cell w-24">種別</th>
                              <th className="px-4 py-2.5 hidden lg:table-cell w-16 text-center">点数</th>
                              <th className="px-4 py-2.5 hidden md:table-cell w-24">希望撮影日</th>
                              <th className="px-4 py-2.5 hidden md:table-cell w-24">納品希望日</th>
                              <th className="px-4 py-2.5 w-8"></th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-800">
                            {active.map(r => (
                              <tr key={r.id} className="hover:bg-gray-800/40 group transition-colors">
                                <td className="px-4 py-3">
                                  <button onClick={() => cyclePhotoStatus(r.id)}
                                    className={`text-xs px-2 py-0.5 rounded-full border cursor-pointer transition-opacity hover:opacity-80 ${PHOTO_STATUS_CLS[r.ステータス]}`}
                                    title="クリックで次のステータスへ">
                                    {r.ステータス}
                                  </button>
                                </td>
                                <td className="px-4 py-3">
                                  <p className="font-medium text-white text-sm">{r.タスク名}</p>
                                  {r.撮影メモ && <p className="text-xs text-gray-500 truncate max-w-xs mt-0.5">{r.撮影メモ}</p>}
                                </td>
                                <td className="px-4 py-3 text-xs text-gray-400 hidden md:table-cell">{r.プロジェクト}</td>
                                <td className="px-4 py-3 text-xs text-gray-400 hidden lg:table-cell">{r.撮影種別}</td>
                                <td className="px-4 py-3 text-xs text-gray-400 text-center hidden lg:table-cell">{r.撮影点数}</td>
                                <td className="px-4 py-3 text-xs text-gray-400 hidden md:table-cell">{r.希望撮影日 || "—"}</td>
                                <td className="px-4 py-3 text-xs text-gray-400 hidden md:table-cell">{r.納品希望日 || "—"}</td>
                                <td className="px-4 py-3">
                                  <button onClick={() => deletePhotoRequest(r.id)}
                                    className="text-gray-700 hover:text-red-400 text-xs cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                    ...
                    !showPhotoForm && <p className="text-center py-8 text-gray-600 text-sm bg-gray-900 rounded-xl border border-gray-800 mb-3">対応中の撮影依頼はありません</p>
                  )}
                  ...
                  {completed.length > 0 && (
                    <div>
                      <button onClick={() => setShowPhotoCompleted(v => !v)}
                        className="flex items-center gap-2 text-xs text-gray-600 hover:text-gray-400 cursor-pointer mb-2 transition-colors">
                        <span>{showPhotoCompleted ? "▾" : "▸"}</span>
                          ...
                          <div className="divide-y divide-gray-800/50">
                            {completed.map(r => (
                              <div key={r.id} className="flex items-center gap-4 px-4 py-2.5 group">
                                <span className={`text-xs px-2 py-0.5 rounded-full border flex-shrink-0 ${PHOTO_STATUS_CLS["完了"]}`}>完了</span>
                                <span className="text-xs text-gray-500 line-through flex-1">{r.タスク名}</span>
                                <span className="text-xs text-gray-600 hidden md:block">{r.プロジェクト}</span>
                                <span className="text-xs text-gray-600 hidden md:block">{r.受付日}</span>
                                <button onClick={() => deletePhotoRequest(r.id)}
                                  className="text-gray-700 hover:text-red-400 text-xs cursor-pointer opacity-0 group-hover:opacity-100">✕</button>
                              </div>
                            ))}
                          </div>
                        </div>
              ...
              return (
                <section>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">🛒</span>
                      <h2 className="text-sm font-semibold text-white">EC依頼</h2>
                      {active.length > 0 && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-blue-900/60 text-blue-300 border border-blue-800/50">
                          {active.length} 件対応中
                        </span>
                      )}
                    </div>
                    <button onClick={() => { setShowEcForm(v => !v); setEcForm(EMPTY_EC()); }}
                      className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg bg-blue-800/70 hover:bg-blue-700/70 text-blue-100 transition-colors cursor-pointer">
                      ＋ 追加
                    </button>
                  </div>
                  ...
                  {/* 追加フォーム */}
                  {showEcForm && (
                    <div className="bg-gray-900 border border-blue-900/50 rounded-xl p-5 space-y-4 mb-4">
                      <h3 className="text-xs font-semibold text-blue-300 uppercase tracking-wide">新規EC依頼</h3>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        <div><label className={labelCls}>受付日</label>
                        ...
                        <div className="col-span-2 md:col-span-3"><label className={labelCls}>依頼内容 *</label>
                          <textarea className={`${inputCls} resize-none`} rows={2} value={ecForm.依頼内容}
                        ...
                        <div className="col-span-2 md:col-span-1"><label className={labelCls}>備考</label>
                      ...
                      <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowEcForm(false)}
                          className="bg-gray-700 hover:bg-gray-600 text-gray-300 px-4 py-2 rounded-lg text-sm cursor-pointer">キャンセル</button>
                        <button onClick={addEcRequest}
                          className="bg-blue-700 hover:bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-medium cursor-pointer">追加</button>
                      </div>
                    </div>
                  )}
                  ...
                  {active.length > 0 ? (
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b border-gray-800 text-left text-xs font-medium text-gray-500">
                              <th className="px-4 py-2.5 w-24">ステータス</th>
                              <th className="px-4 py-2.5 hidden md:table-cell w-24">種別</th>
                              <th className="px-4 py-2.5">依頼内容</th>
                              <th className="px-4 py-2.5 hidden lg:table-cell w-20">依頼者</th>
                              <th className="px-4 py-2.5 hidden md:table-cell w-20">対象サイト</th>
                              <th className="px-4 py-2.5 hidden md:table-cell w-24">期日</th>
                              <th className="px-4 py-2.5 w-8"></th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-800">
                                ...
                                <tr key={r.id} className="hover:bg-gray-800/40 group transition-colors">
                                  <td className="px-4 py-3">
                                    <button onClick={() => cycleEcStatus(r.id)}
                                      className={`text-xs px-2 py-0.5 rounded-full border cursor-pointer transition-opacity hover:opacity-80 ${EC_STATUS_CLS[r.ステータス]}`}
                                      title="クリックで次のステータスへ">
                                      {r.ステータス}
                                    </button>
                                  </td>
                                  <td className="px-4 py-3 text-xs text-gray-400 hidden md:table-cell">{r.種別}</td>
                                  <td className="px-4 py-3">
                                    <p className="text-sm text-white font-medium truncate max-w-xs">{r.依頼内容}</p>
                                    {r.備考 && <p className="text-xs text-gray-500 truncate mt-0.5">{r.備考}</p>}
                                  </td>
                                  <td className="px-4 py-3 text-xs text-gray-400 hidden lg:table-cell">{r.依頼者}</td>
                                  <td className="px-4 py-3 text-xs text-gray-400 hidden md:table-cell">{r.対象サイト}</td>
                                  <td className={`px-4 py-3 text-xs hidden md:table-cell font-medium ${overdue ? "text-red-400" : "text-gray-400"}`}>
                                    {r.期日 || "—"}{overdue && " ⚠"}
                                  </td>
                                  <td className="px-4 py-3">
                    ...
                    !showEcForm && <p className="text-center py-8 text-gray-600 text-sm bg-gray-900 rounded-xl border border-gray-800 mb-3">対応中のEC依頼はありません</p>
                  )}
                      ...
                      {showEcCompleted && (
                        <div className="bg-gray-900/50 rounded-xl border border-gray-800/50 overflow-hidden">
                          <div className="divide-y divide-gray-800/50">
                            {completed.map(r => (
                              <div key={r.id} className="flex items-center gap-4 px-4 py-2.5 group">
                                <span className={`text-xs px-2 py-0.5 rounded-full border flex-shrink-0 ${EC_STATUS_CLS["完了"]}`}>完了</span>
                                <span className="text-xs text-gray-500 line-through flex-1 truncate">{r.依頼内容}</span>
                                <span className="text-xs text-gray-600 hidden md:block">{r.対象サイト}</span>
                                <span className="text-xs text-gray-600 hidden md:block">{r.完了日 || r.受付日}</span>
                                <button onClick={() => deleteEcRequest(r.id)}
                                  className="text-gray-700 hover:text-red-400 text-xs cursor-pointer opacity-0 group-hover:opacity-100">✕</button>
                              </div>
                            ))}
