...
const initialLogs = [
  { id: 1, 日付: "2026-05-28", プロジェクト名: "グラビングシューズ", フェーズ: "企画", 進捗内容: "キックオフMTG実施。ターゲット・競合調査方針決定", 関係者: "田中・佐藤", 次のマイルストーン: "競合調査完了（6/5）" },
  { id: 2, 日付: "2026-05-27", プロジェクト名: "Angle 3D", フェーズ: "制作", 進捗内容: "GAIA-MAX LP構成案作成開始", 関係者: "山田", 次のマイルストーン: "LP企画書提出（6/5）" },
  { id: 3, 日付: "2026-05-27", プロジェクト名: "マテリアル", フェーズ: "レビュー", 進捗内容: "稟議書v9提出。部長レビュー待ち", 関係者: "部長・山本", 次のマイルストーン: "承認（6/1）" },
];
        ...
        {activeTab === "logs" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-gray-400 text-sm">{logs.length} 件の進捗ログ</p>
            ...
            <div className="space-y-2">
              {logs.map((l, i) => (
                <div key={l.id ?? i} className="bg-gray-900 rounded-xl border border-gray-800 px-5 py-4 group">
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="text-xs text-gray-500">{l.日付}</span>
                      <span className="text-sm font-medium text-teal-400">{l.プロジェクト名}</span>
                      <span className="text-xs px-2 py-0.5 bg-gray-800 rounded-full text-gray-400">{l.フェーズ}</span>
                  ...
                  {l.次のマイルストーン && (
                    <p className="text-xs text-yellow-400/80">
                      <span className="text-yellow-600 mr-1">→</span>{l.次のマイルストーン}
                    </p>
                  )}
                </div>
              ))}
              {logs.length === 0 && <p className="text-center py-12 text-gray-500 text-sm">進捗ログがありません</p>}
            </div>
          </div>
        )}
      </main>
